...
foreach my $mailoutid (keys %mailout) {
    print "LockReadConfirmationsByMailout $mailoutid\n";
    my $campaign_id = $mailout{$mailoutid}->{campaign_id};
    my $list_id = $mailout{$mailoutid}->{list_id};
    my $batchid = $MailoutReporter->LockReadConfirmationsByMailout($mailoutid, 10, 2000);
...
foreach my $mailoutid (keys %mailout) {
    print "LockUrlClicksByMailout $mailoutid\n";
    my $campaign_id = $mailout{$mailoutid}->{campaign_id};
    my $list_id = $mailout{$mailoutid}->{list_id};
    my $batchid = $MailoutReporter->LockUrlClicksByMailout($mailoutid, 10, 2000);
    my @info = $MailoutReporter->GetLockedUrlClicks($batchid);
    my $total = scalar(@info); my $flag = 0;
    foreach my $i (@info) {
        $flag++;
        print "$flag/$total on $i->{Subscriberid}\n";
